...
#include "Components/EnhancedInputAbilitySystem.h"
...
/*
void UEnhancedInputAbilitySystem::ResetBindings()
{
	for (auto& InputBinding : MappedAbilities)
	{
		...
		for (FGameplayAbilitySpecHandle AbilityHandle : InputBinding.Value.BoundAbilitiesStack)
		{
			...
			{
				FoundAbility->InputID = EnhancedInputAbilitySystem_Impl::InvalidInputID;
			}
		}
	}
}

void UEnhancedInputAbilitySystem::SetupPlayerControls_Implementation(UEnhancedInputComponent* PlayerInputComponent)
{
	ResetBindings();
	...
	for (auto& Ability : MappedAbilities)
	{
		...
		// Pressed event
		InputComponent->BindAction(InputAction, ETriggerEvent::Started, this, &UEnhancedInputAbilitySystem::OnAbilityInputPressed, InputAction);

		// Released event
		InputComponent->BindAction(InputAction, ETriggerEvent::Completed, this, &UEnhancedInputAbilitySystem::OnAbilityInputReleased, InputAction);
	}

	RunAbilitySystemSetup();
}

void UEnhancedInputAbilitySystem::ReleaseInputComponent(AController* OldController)
{
	ResetBindings();
